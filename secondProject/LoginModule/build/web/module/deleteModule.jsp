...
<html>
    ...
    <body>
        ...
        <sql:query dataSource="${ds}" scope="session" var="rs">
            select * from Batch where Module_ID=?
            <sql:param value="${param.module_id}"/>
        </sql:query>
        ...
        <c:if test="${exists > 0}">
            <jsp:forward page="index.jsp">
                <jsp:param name="status" value="Can not delete the Module !! Batch exists for this module !!" />
            </jsp:forward>
        </c:if>
        ...
        <c:if test="${result <= 0}">
            <jsp:forward page="index.jsp">
                <jsp:param name="status" value="Could not delete the Module !!" />
            </jsp:forward>
        </c:if>
    </body>
</html>
